 ...
 *   GET /api/run?key=$CRON_SECRET&dry=1&date=2026-09-18&throughDate=2026-09-21
 *                                                     Fri–Mon backlog window
...
export default async function handler(req, res) {
  ...
  const throughDate = queryParam(req, 'throughDate') || queryParam(req, 'through') || null;

  try {
    const stats = await runFollowUp({ dryRun, date, throughDate });
