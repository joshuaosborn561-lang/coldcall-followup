...
export async function runFollowUp({ dryRun = false, date = null, tz = undefined } = {}) {
  ...
  // Group the survivors by campaign. Two numbers may share a campaign, so key
  // on campaignId rather than on the route.
  const byCampaign = new Map();
  for (const entry of byEmail.values()) {
    const bucket = byCampaign.get(entry.route.campaignId) || { leads: [], routes: new Set() };
    bucket.leads.push(toLead(entry));
  ...
  stats.leads = [...byEmail.values()].map(({ email, contact, route }) => ({
    email,
 ...
 * campaign, or Smartlead drops it -- see the README. `called_by` lets one
 * shared campaign still personalise by rep if you ever collapse the split.
 */
