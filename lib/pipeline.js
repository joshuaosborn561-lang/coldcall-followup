...
export async function runFollowUp({ dryRun = false, date = null, tz = undefined } = {}) {
  ...
  for (const call of voicemails) {
    ...
    const email = firstEmail(person?.emails) || firstEmail(extracted?.emails);
    ...
    const record = {
      ...
      extracted,
      email,
      source: firstEmail(person?.emails) ? 'allo_crm' : email ? 'call_extraction' : null,
    };
...
function extractedContact(call) {
  ...
  return {
    name: clean(c.name),
    emails: (c.emails || []).filter((e) => typeof e === 'string' && e.includes('@')),
...
function companyName(record) {
  return record.person?.company?.name || record.extracted?.company || '';
}
