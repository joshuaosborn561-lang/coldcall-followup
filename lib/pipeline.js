/**
 * The job: Allo no-connect / voicemail calls -> Smartlead campaign.
 *
 *   1. Work out the time window (recent lookback, or a calendar day for
 *      manual `/api/run?date=`).
 *   2. Pull OUTBOUND calls from Allo v2 in that window.
 *   3. Keep no-connects and voicemail hits; drop real live conversations.
...
import { formatCallMoment, zonedDayWindow, zonedLookbackWindow } from './time.js';
...
export async function runFollowUp({
  ...
  const window =
    lookbackMinutes != null && date == null
      ? zonedLookbackWindow(lookbackMinutes, tz)
      : zonedDayWindow(date, tz, throughDate);
  ...
  const stats = {
    ...
    timezone: window.tz,
    mode: window.mode,
    lookbackMinutes: window.lookbackMinutes ?? null,
  ...
  // 1-2. Outbound calls in the window, across every Allo number.
  ...
  // 3. No-connect + voicemail. Live conversations are dropped with a reason.
  const eligible = [];
  for (const call of calls) {
    const verdict = shouldFollowUp(call);
    if (verdict.kind === 'uncertain') stats.totals.uncertain++;
    if (verdict.include) {
      eligible.push(call);
      const rep = repName(call);
      stats.byRep[rep] = (stats.byRep[rep] || 0) + 1;
      if (verdict.kind === 'no_connect') stats.totals.noConnect++;
      if (verdict.kind === 'voicemail') stats.totals.voicemail++;
    } else {
      stats.skipped.push({
        number: call.contact_number,
        calledBy: repName(call),
        reason: `${verdict.kind === 'conversation' ? 'live conversation' : 'not eligible'} — ${verdict.reason}`,
        callId: call.id,
        kind: verdict.kind,
      });
    }
  }
  stats.totals.eligibleCalls = eligible.length;
  stats.totals.voicemailsLeft = eligible.length;
  ...
  const wanted = new Set(eligible.flatMap((c) => phoneKeys(c.contact_number)));
  ...
  stats.totals.peopleReached = new Set(eligible.map((c) => c.contact_number)).size;
  ...
  // 5. One follow-up per person, newest eligible call wins.
  ...
  stats.leads = [...byEmail.values()].map((r, i) => ({
    ...
    calledAt: leads[i].custom_fields.call_time,
