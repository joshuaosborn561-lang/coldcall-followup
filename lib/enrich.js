...
const PROVIDERS = [
  {
    // Base confirmed live: app.getleads.io/api/v1 with Bearer auth (NOT
    // api.getleads.io, which does not resolve). Field names come from the
    // getleads MCP search_contacts schema, which the REST route mirrors.
    // Billing is 1 credit per record returned, 0 if none.
    name: 'getleads',
    verified: true,
