/**
 * Optional Slack ping so an unattended job is not invisible.
...
export function formatSummary(stats) {
  ...
  const eligible = totals.eligibleCalls ?? totals.voicemailsLeft ?? 0;
  lines.push(
    `${eligible} no-connect/voicemail call${eligible === 1 ? '' : 's'} ` +
      `of ${totals.callsInWindow ?? 0} outbound · ` +
  ...
  // Only the no-email cases are worth listing; live conversations are
  ...
  if (noEmail.length) {
    ...
    lines.push(`\n*Eligible but no email found (${noEmail.length}):*`);
